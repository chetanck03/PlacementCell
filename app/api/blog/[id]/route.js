...
export async function PUT(req, res) {
  ...
  try {
    ...
    if (blog?.authorId?._id.toString() !== decodedToken._id.toString()) {
      return NextResponse.json(
        { msg: "Only author can update his/her post" },
        { status: 403 }
      );
    }
...
export async function DELETE(req, res) {
  ...
  try {
    const post = await Blog.findById(id).populate("authorId");

    if (post?.authorId?._id.toString() !== decodedToken._id.toString()) {
      return NextResponse.json(
        { msg: "Only author can delete his/her post" },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({ msg: "Successfully deleted post" }, { status: 200 });
  } catch (error) {
