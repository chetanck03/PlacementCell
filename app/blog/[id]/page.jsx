...
const BlogDetails = ({ params }) => {
  ...
  const timeStr = postDetails?.createdAt;
  const time = moment(timeStr);
  const formattedTime = time.format("MMMM Do YYYY");

  const handlepostDelete = async (imageId) => {
    try {
  ...
  return (
    <section className="container max-w-3xl">
      {postDetails?.authorId?._id.toString() ===
        session?.user?._id.toString() && (
          <div className="flex items-center justify-end gap-5">
            ...
            <button
              onClick={() => handlepostDelete(postDetails?.image?.id)}
      ...
      <div className="flex flex-col items-center justify-center">
        <Link href={`/user/${postDetails?.authorId?._id.toString()}`}>
          <div className="flex flex-col justify-center items-center py-10">
            <Image
              src={
                postDetails?.authorId?.avatar?.url
                  ? postDetails?.authorId?.avatar?.url
                  : demoImage
              }
              alt="avatar image"
            ...
            <div className="text-center">
              <p className="text-whiteColor">{postDetails?.authorId?.name}</p>
              <p>{postDetails?.authorId?.designation}</p>
            </div>
          </div>
        </Link>
        ...
        <div className="text-center space-y-3">
          ...
          <p className="flex items-center justify-center gap-3">
            <span className="text-blue-500">{postDetails?.category}</span>
          ...
          <div className="flex flex-col justify-center items-center ">
            <Image
              src={postDetails?.image ? postDetails?.image?.url : demoImage}
              alt="post details image"
              width={0}
              height={0}
              sizes="100vw"
              className="w-[500px] h-[500px]  rounded-lg py-10"
            />
          </div>
          ...
          <div className="text-start">
            <div className="space-y-5">
              {postDetails?.description &&
                splitParagraph(postDetails?.description).map(
                  (paragraph, pIndex) => (
                    <div key={pIndex}>
                      {pIndex ===
                        Math.floor(
                          splitParagraph(postDetails?.description).length / 2
                        ) && (
                          <blockquote className="border-l-4 border-blue-500 border-spacing-6 italic mb-5">
                            <p className="ml-5">{postDetails?.quote}</p>
                          </blockquote>
                        )}
